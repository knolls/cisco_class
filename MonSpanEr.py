	...
	while flag:
		...
		for i in  interfacers:
			...
			if x.eth_outpkts > 1000:
				errSpanSource = x.interface
				print "Over 1000 packets on :", x.interface, "Packet number is:",x.eth_outpkts
				sw1.conf(" monitor session 30 type erspan-source ; description ERSPAN to 30 to POC ; source interface " + errSpanSource + " both ; destination ip " + span_dest_ip + " ; erspan-id 30 ; ip ttl 64 ; ip prec 0 ; ip dscp 0 ; mtu 1500 ; header-type 2 ; no shut ;")
				print "\tBeginning Cap for 1 min"
				time.sleep(60)
				print "\tDone capturing, now Cleaning config"
		...
		time.sleep(3)
		print "------------"
		print max(allPack), "packets number under 1000"
		print "------------"
...
def main():
	...
	for device in devices_Crawled:
		for vrf in device.vrfList:
			print vrf.name,device.hostname
...
#sw1.conf(" monitor session 30 type erspan-destination ; description ERSPAN to 30 to POC ; erspan-id 30 ; vrf " + VVRFNAME +  " ; source ip " + IP + " ; destination "  +  DES  + " ; no shut ;")
